...
export const useToolkit = () => {
    ...
    const toolkitDefinitions = [
        ...
        { type: 'function', name: 'reply_to_email', description: 'Replies to an existing email. The emailId MUST be a real ID obtained from a previous tool result.', parameters: { type: 'object', properties: { emailId: { type: 'string' }, text: { type: 'string', description: 'Reply body text' } }, required: ['emailId', 'text'] } },
        { type: 'function', name: 'get_email_by_id', description: 'Retrieves a specific email by its unique ID. The emailId MUST be a real ID obtained from get_emails or search_emails.', parameters: { type: 'object', properties: { emailId: { type: 'string' } }, required: ['emailId'] } },
